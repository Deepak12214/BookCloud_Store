...
function Books() {
  ...
  return (
    <div className="flex mt-16 justify-around">
      {(user == undefined || null) || (Book == null) ? (
        <div className='min-h-96 pt-10 text-xl'>
          {user == undefined || null ? ('You have to login  🫠🫠') : ('Check Your Internet 🫣🫣')}
        </div>
      ) : (
...
{genres.map((genre) => {
              return (
                <div className="hover:bg-slate-800 p-2 rounded-lg text-xl my-3" key={genre}>
                  <NavLink
                    to={`/books/${genre}`}
                    className={({ isActive }) => (isActive ? 'text-purple-700 focus:text-purple-600 font-bold' : '')}
                  >
                    {genre}
                  </NavLink>
                </div>
          ...
          <div className="right_container w-full lg:w-9/12  py-4  lg:h-[90vh] lg:overflow-y-scroll hide-scroll rounded-lg bg-gradient-to-b from-base-100 to-slate-900 ">
            <div className="flex flex-wrap justify-center gap-4">
              {isLoading ? (
                Array(10).fill(0).map((_, index) => <ShimmerCart key={index} />)
              ) : Book != null ? (
                Book.map((data) => {
                  return (
